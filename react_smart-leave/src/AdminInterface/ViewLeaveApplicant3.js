...
function LeaveApplicationsByDate() {
    ...
    const isNameAndRoleValid = (name, role) => {
        if (!name || !role || !membersData || !Array.isArray(membersData)) return false;
        return membersData.some(member => 
            member.fullName.toLowerCase() === name.trim().toLowerCase() && 
    ...
    // Handle taking action 3
    const handleTakeAction3 = (application) => {
        setSelectedApplication(application);
    ...
    return (
        <div className="leave-applications-container">
            ...
            <div className="container-fluid p-5">
                                            ...
                                            <tr key={app._id}>
                                                ...
                                                <td>
                                                    <div className="action-buttons d-flex flex-column gap-2">
                                                        ...
                                                        {/* Action 2 Button (always visible but disabled) */}
                                                        <Button 
                                                            variant="warning" 
                                                            disabled
                                                            className="disabled-action"
                                                      ...
                                                      {/* Action 3 Button */}
                                                        <>
                                                            <Button 
                                                                variant="success" 
                                                                onClick={() => handleTakeAction3(app)}
                                                                disabled={
                                                                    !actionStatus[app._id]?.isCompleted1 || 
                                                                    !actionStatus[app._id]?.isCompleted2 || 
                                                                    actionStatus[app._id]?.isCompleted3
                                                                }
                                                                className={
                                                                    actionStatus[app._id]?.isCompleted3 ? "completed" : ""
                                                                }
                                                            >
                                                                Action 3
                                                            </Button>

                                                            {/* Show message only if all requirements are not met */}
                                                            {(!actionStatus[app._id]?.isCompleted1 || !actionStatus[app._id]?.isCompleted2) && (
                                                                <div className="action-message warning">
                                                                    {!actionStatus[app._id]?.isCompleted1 && !actionStatus[app._id]?.isCompleted2 ? 
                                                                        "Actions 1 and 2 not completed yet" :
                                                                        !actionStatus[app._id]?.isCompleted1 ? 
                                                                        "Action 1 not completed yet" : 
                                                                        "Action 2 not completed yet"}
                                                                </div>
                                                            )}
                ...
                <Modal show={showActionModal2} onHide={() => { setShowActionModal2(false); resetForm(); }} centered size="lg" className="action-modal">
                    ...
                    <Modal.Body>
                        <Form className="action-form">
                            <Form.Group>
                                <Form.Label>Approve by Leave Clerck</Form.Label>
                                <Form.Control 
                                    as="select" 
                                    value={finalApproval} 
                ...
                {/* Action Modal 3 with Digital Signature */}
                <Modal show={showActionModal3} onHide={() => { setShowActionModal3(false); resetForm(); }} centered size="lg" className="action-modal">
                    <Modal.Header closeButton>
                        <Modal.Title>Take Action 3 - Final Approval</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Form className="action-form">
                            <Form.Group>
                    ...
                    <Modal.Footer>
                        ...
                        <Button variant="primary" onClick={handleSubmit3} className="submit-btn">
                            Submit with Signature
                        </Button>
                    </Modal.Footer>
                </Modal>
